...
class BaseSchema(BaseModel):
    ...
    tone: str = Field(..., description="The best tone and style to use in communication given customer's message to maintain engagement.")
...
class ProductInfo(BaseSchema):
    ...
    miscellaneous: Optional[str] = Field(..., description="Additional info about the product as a list of ',' separated values")
    product_category: Optional[str] = Field(..., description="product category e.g fashion, furniture etc")
    intent: str = Field(..., description="Customer's Intent: [enquiry, purchase]")
    instruction: Optional[str] = Field(..., description="Additonal instruction to be used to ensure better service to customer.")
    use_internet: bool = Field(..., description="Whether to use internet to fetch information about product description")
...
class Logistics(BaseSchema):
    ...
    customer_address: str = Field(..., description="The customer's address for product delivery")
...
class AdsMarketing(BaseSchema):
    ...
    product_category: str = Field(..., description="Product's category e.g Fashion, health etc")
    accessory: str = Field(..., description="A list of 3 other products or accessories that are typically purchased or used alongside the item purchased by customer.")
...
class CustomerComplaint(BaseSchema):
    ...
    sentiment: Optional[str] = Field(..., description="customer sentiment about product.")
    product_rating: Optional[str] = Field(..., description="Estimated rating of product based on customer message [1 - 10]")
